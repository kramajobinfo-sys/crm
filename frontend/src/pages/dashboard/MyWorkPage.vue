<template>
  <div class="page">
    <div class="page-header"><div><h1 class="page-title">{{ $t('my_work.title') }}</h1><p class="page-sub">{{ $t('my_work.subtitle') }}</p></div><button class="btn-secondary btn-sm" :disabled="loading" @click="load"><RefreshCw :size="12" :class="loading&&'animate-spin'" /> {{ $t('my_work.refresh') }}</button></div>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2.5 mb-3"><div v-for="s in summaryItems" :key="s.key" class="stat"><div class="text-[11px] text-ink-muted">{{ $t(s.label) }}</div><div class="text-lg font-semibold mt-0.5" :class="s.key==='overdue'&&summary[s.key]?'text-red-600':'text-ink dark:text-ink-dark'">{{ summary[s.key]||0 }}</div></div></div>
