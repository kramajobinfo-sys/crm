<template>
  <div class="p-4 md:p-5 max-w-[1600px] mx-auto">
    <div class="flex items-end justify-between gap-3 mb-4">
      <div>
        <h1 class="text-lg font-medium text-ink dark:text-ink-dark">{{ $t('projects.title') }}</h1>
        <p class="text-xs text-ink-muted dark:text-ink-dark-muted mt-0.5">{{ $t('projects.subtitle') }}</p>
      </div>
      <div class="flex flex-wrap justify-end gap-2"><button v-if="can('project_reports.view')" class="btn-secondary text-xs px-3 py-1.5" @click="openReport"><BarChart3 :size="13" /> {{ $t('projects.reports') }}</button><button v-if="can('project_automation.view')" class="btn-secondary text-xs px-3 py-1.5" @click="openAutomations"><Workflow :size="13" /> {{ $t('projects.automation') }}</button><button v-if="can('project_templates.view')" class="btn-secondary text-xs px-3 py-1.5" @click="openTemplates"><Copy :size="13" /> {{ $t('projects.templates') }}</button><button v-if="can('projects.view')" class="btn-secondary text-xs px-3 py-1.5" @click="openWorkload"><Users :size="13" /> {{ $t('projects.workload') }}</button><button v-if="can('projects.create')" class="btn-primary text-xs px-3 py-1.5" @click="openProject()"><Plus :size="13" /> {{ $t('projects.new') }}</button></div>
    </div>

    <div class="grid grid-cols-2 lg:grid-cols-5 gap-2.5 mb-3">
      <div v-for="item in statItems" :key="item.key" class="card p-3">
        <div class="text-[11px] text-ink-muted dark:text-ink-dark-muted">{{ $t(item.label) }}</div>
        <div class="text-lg font-semibold text-ink dark:text-ink-dark mt-0.5">{{ stats[item.key] ?? 0 }}</div>
      </div>
    </div>

    <div class="card p-2.5 mb-3 flex flex-wrap gap-2 items-center">
      <input v-model="filters.q" class="input text-sm w-64" :placeholder="$t('projects.search')" @keyup.enter="loadProjects" />
      <select v-model="filters.status" class="input text-sm w-auto" @change="loadProjects">
      ...
      <button class="btn-secondary text-xs px-2.5 py-1 ml-auto" :disabled="loading" @click="loadAll"><RefreshCw :size="12" :class="loading && 'animate-spin'" /> {{ $t('projects.refresh') }}</button>
    </div>
    ...
    <div class="card overflow-hidden">
      ...
      <div v-else-if="!projects.length" class="py-16 text-center">
        <BriefcaseBusiness :size="28" class="mx-auto text-ink-subtle mb-2" />
        <div class="text-sm text-ink-muted">{{ $t('projects.empty') }}</div>
      </div>
      <div v-else class="overflow-x-auto">
        <table class="w-full text-xs">
          <thead class="bg-slate-50 dark:bg-surface-dark-subtle text-ink-muted"><tr>
            <th class="text-left font-medium px-3 py-2">{{ $t('projects.project') }}</th><th class="text-left font-medium px-3 py-2">{{ $t('projects.account') }}</th><th class="text-left font-medium px-3 py-2">{{ $t('projects.owner') }}</th><th class="text-left font-medium px-3 py-2">{{ $t('projects.status') }}</th><th class="text-left font-medium px-3 py-2 w-44">{{ $t('projects.progress') }}</th><th class="text-left font-medium px-3 py-2">{{ $t('projects.due') }}</th>
          </tr></thead>
          <tbody><tr v-for="project in projects" :key="project.id" class="border-t border-slate-100 dark:border-slate-700/60 hover:bg-slate-50 dark:hover:bg-surface-dark-subtle cursor-pointer" @click="openDetail(project.id)">
            <td class="px-3 py-2.5"><div class="font-medium text-ink dark:text-ink-dark">{{ project.name }}</div><div class="font-mono text-[10px] text-ink-subtle">{{ project.project_no }}</div></td>
            <td class="px-3 py-2.5 text-ink-muted">{{ project.customer?.name || '—' }}</td><td class="px-3 py-2.5 text-ink-muted">{{ project.owner?.name || '—' }}</td>
            <td class="px-3 py-2.5"><span class="px-1.5 py-0.5 rounded text-[10px]" :class="statusClass(project.status)">{{ label(project.status) }}</span></td>
            <td class="px-3 py-2.5"><div class="flex items-center gap-2"><div class="h-1.5 flex-1 rounded bg-slate-200 dark:bg-slate-700"><div class="h-full rounded bg-primary-500" :style="{ width: `${project.progress}%` }" /></div><span class="tabular-nums text-[10px] text-ink-muted">{{ project.progress }}%</span></div></td>
            <td class="px-3 py-2.5" :class="isOverdue(project) ? 'text-red-600' : 'text-ink-muted'">{{ project.due_date || '—' }}</td>
          </tr></tbody>
        </table>
      </div>
    </div>
    ...
    <div v-if="selected" class="fixed inset-0 z-40 flex justify-end bg-black/30" @click.self="selected=null">
      <div class="bg-white dark:bg-surface-dark-muted w-full md:w-[44rem] h-full shadow-xl flex flex-col overflow-hidden">
        <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-700 flex items-start gap-2">
          ...
          <button v-if="can('projects.update')" class="btn-secondary text-[11px] px-2 py-1" @click="openProject(selected)">{{ $t('projects.edit') }}</button>
    ...
    <div v-if="reportModal" class="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" @click.self="reportModal=false"><div class="card w-full max-w-5xl max-h-[94vh] overflow-y-auto p-4"><div class="flex flex-wrap items-center gap-2 mb-4"><div class="mr-auto"><h2 class="text-sm font-medium">{{ $t('projects.management_report') }}</h2><p class="text-[11px] text-ink-subtle">{{ $t('projects.report_help') }}</p></div><input v-model="reportFilters.from" type="date" class="input text-xs w-auto" /><input v-model="reportFilters.to" type="date" class="input text-xs w-auto" /><button class="btn-secondary text-xs px-2 py-1" @click="loadReport"><RefreshCw :size="12" /></button><button @click="reportModal=false"><X :size="15" /></button></div>
      ...
      <div class="overflow-x-auto"><table class="w-full text-xs"><thead class="bg-slate-50 dark:bg-surface-dark-subtle"><tr><th class="text-left p-2">{{ $t('projects.project') }}</th><th class="text-left p-2">{{ $t('projects.status') }}</th><th class="text-right p-2">{{ $t('projects.progress') }}</th><th class="text-right p-2">{{ $t('projects.overdue_tasks') }}</th><th class="text-right p-2">{{ $t('projects.actual_cost') }}</th><th class="text-right p-2">{{ $t('projects.budget') }}</th></tr></thead><tbody><tr v-for="p in report.projects" :key="p.id" class="border-t border-slate-100 dark:border-slate-700"><td class="p-2"><button class="text-left text-primary-600" @click="openReportProject(p)">{{ p.project_no }} · {{ p.name }}</button></td><td class="p-2">{{ label(p.status) }}</td><td class="p-2 text-right">{{ p.progress }}%</td><td class="p-2 text-right" :class="p.overdue_tasks?'text-red-600':''">{{ p.overdue_tasks }}</td><td class="p-2 text-right">{{ money(p.actual_cost,p.currency) }}</td><td class="p-2 text-right">{{ money(p.budget,p.currency) }}</td></tr></tbody></table></div>
    </div></div>
      ...
      <div class="card w-full max-w-3xl max-h-[94vh] flex flex-col overflow-hidden">
        ...
        <div class="p-4 overflow-y-auto flex-1">
          <form v-if="taskTab==='details'" class="space-y-3" @submit.prevent="saveTask">
            ...
            <div class="flex justify-end"><button v-if="can('project_tasks.update')" class="btn-primary text-xs px-3 py-1.5" :disabled="taskModal.saving">{{ taskModal.saving?$t('projects.saving'):$t('projects.save_task') }}</button></div>
    ...
    <div v-if="projectForm.open" class="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" @click.self="projectForm.open=false">
      <form class="card w-full max-w-xl p-4 space-y-3 max-h-[90vh] overflow-y-auto" @submit.prevent="saveProject">
        ...
        <div class="flex justify-end gap-2"><button type="button" class="btn-secondary text-xs px-3 py-1.5" @click="projectForm.open=false">{{ $t('projects.cancel') }}</button><button class="btn-primary text-xs px-3 py-1.5" :disabled="projectForm.saving">{{ projectForm.saving ? $t('projects.saving') : $t('projects.save') }}</button></div>
      </form>
    </div>
  </div>
</template>
